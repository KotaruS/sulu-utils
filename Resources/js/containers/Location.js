...
class Location extends React.Component<Props> {
  ...
  @computed get label() {
    ...
    return translate('sulu_utils.select_location');
  }
  ...
  componentDidUpdate(prevProps: Props) {
    ...
    if (!equals(prevValue, newValue) && newValue && this.map) {
      this.map.setView([newValue.lat ?? this.props.center[0] ?? 0, newValue.long ?? this.props.center[1] ?? 0], newValue.zoom ?? this.props.defaultZoom ?? 10);
    }
  }
  ...
  @computed get mapCenter() {
    if (this.props.value) {
      return [this.props?.value.lat, this.props?.value.long]
    }
      ...
      <div className={locationClass}>
        ...
        {value &&
          ...
          >
            ...
            <Marker interactive={false} position={this.mapCenter}>
